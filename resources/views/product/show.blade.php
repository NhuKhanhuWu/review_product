...
@section('content')
    ...
    <div class="center-x-50 mt-5">
        ...
        <div class="gap-5 flex top-12 sticky-header top-3rem py-2">
            <p class="font-bold mb-2">Filter by:</p>

            <x-sort_filter.sort-filter :method="$filter" :hostName="'product'" :hostValue="$product" :routeName="'products.show'"
                :feature="'filter'">
        ...
        <form class="my-5 flex flex-col gap-3"
            action="{{ route('reviews.store', ['product_id' => $product, 'user_id' => Auth::id()]) }}" method="POST">
            @csrf
